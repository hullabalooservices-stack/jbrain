...
- `src/core/minions/` — Minions job queue: BullMQ-inspired, Postgres-native (queue, worker, backoff, types). Layout as of v0.13.1: `queue.ts`, `worker.ts`, `types.ts`, `attachments.ts`, `backoff.ts`, `index.ts`. No shell handler, no protected-names module, no quiet-hours/stagger columns — those were v0.13-development scaffolding that did not ship.
- `src/core/minions/queue.ts` — MinionQueue class (submit, claim, complete, fail, stall detection, parent-child, depth/child-cap, per-job timeouts, cascade-kill, attachments, idempotency keys, child_done inbox, removeOnComplete/Fail). `add()` plumbs `max_stalled` through with a `[1, 100]` clamp (v0.13.1 #219).
- `src/core/minions/worker.ts` — MinionWorker class (handler registry, lock renewal, graceful shutdown, timeout safety net)
- `src/core/minions/types.ts` — `MinionJobInput` + `MinionJobStatus` + handler context types. `MinionJobInput.max_stalled` (new in v0.13.1) is optional and defaults to the schema value (5).
...
- `src/commands/migrations/` — TS migration registry (compiled into the binary; no filesystem walk of `skills/migrations/*.md` needed at runtime). `index.ts` lists migrations in semver order. Current entries: `v0_11_0.ts` (Minions adoption, 8 phases), `v0_12_0.ts` (Knowledge Graph auto-wire, 5 phases; `phaseASchema` timeout bumped to 600s in v0.12.1 for duplicate-heavy brains), `v0_12_2.ts` (JSONB double-encode repair, 4 phases), `v0_13_0.ts` (frontmatter edge projection). v0.13.1 ships as schema-only migrations (v12 `pages_updated_at_index` + v13 `minion_jobs_max_stalled_default_5`) driven by the `MIGRATIONS` array in `src/core/migrate.ts` — no orchestrator needed because there are no phase-level operations beyond the SQL itself. All orchestrators are idempotent and resumable from `partial` status.
...
- `src/commands/doctor.ts` — `gbrain doctor [--json] [--fast] [--fix] [--index-audit]`: health checks. v0.12.3 added two reliability checks: `jsonb_integrity` (scans pages.frontmatter, raw_data.data, ingest_log.pages_updated, files.metadata for `jsonb_typeof='string'` rows left over from v0.12.0) and `markdown_body_completeness` (flags pages whose compiled_truth is <30% of raw source when raw has multiple H2/H3 boundaries). v0.13.1: `schema_version` check now fails loudly when `version=0` (migrations never ran — the #218 `bun install -g` signature) and routes users to `gbrain apply-migrations --yes`. New opt-in `--index-audit` flag (Postgres-only) reports zero-scan indexes from `pg_stat_user_indexes` — informational only, no auto-drop. Fix hints point at `gbrain repair-jsonb`, `gbrain sync --force`, and `gbrain apply-migrations`.
- `src/core/migrate.ts` — schema-migration runner. Owns the `MIGRATIONS` array (source of truth for schema DDL). v0.13.1 extended the `Migration` interface with `sqlFor?: { postgres?, pglite? }` (engine-specific SQL overrides `sql`) and `transaction?: boolean` (set to false for `CREATE INDEX CONCURRENTLY`, which Postgres refuses inside a transaction; ignored on PGLite since it has no concurrent writers). Migration v12 uses a handler branching on `engine.kind` to run CONCURRENTLY on Postgres (with a pre-drop of any invalid remnant) and plain `CREATE INDEX` on PGLite.
...
- `scripts/check-jsonb-pattern.sh` — CI grep guard. Fails the build if anyone reintroduces (a) the `${JSON.stringify(x)}::jsonb` interpolation pattern (postgres.js v3 double-encodes it), or (b) `max_stalled INTEGER NOT NULL DEFAULT 1` in any schema source file (v0.13.1 #219 regression guard — must be DEFAULT 5 to preserve SIGKILL-rescue). Wired into `bun test`.
...
Key commands added in v0.13.1 (fix wave):
...
- `gbrain jobs smoke --sigkill-rescue` — opt-in regression smoke case simulating a killed worker; asserts the v0.13.1 schema default (`max_stalled=5`) actually rescues on first stall.
