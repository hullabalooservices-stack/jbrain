/**
 * Tests for the v0.11.2 Knowledge Graph auto-wire orchestrator.
 *
...
describe('v0.11.2 — Knowledge Graph auto-wire migration', () => {
  test('registered in the TS migration registry', async () => {
    ...
    expect(versions).toContain('0.11.2');
    const m = getMigration('0.11.2');
  ...
  test('feature pitch includes the headline benchmark numbers', async () => {
    const { v0_11_2 } = await import('../src/commands/migrations/v0_11_2.ts');
    const desc = v0_11_2.featurePitch.description ?? '';
  ...
  test('phase functions exported for unit testing', async () => {
    const { __testing } = await import('../src/commands/migrations/v0_11_2.ts');
  ...
  test('dry-run skips all side-effect phases', async () => {
    const { v0_11_2 } = await import('../src/commands/migrations/v0_11_2.ts');
    const result = await v0_11_2.orchestrator({
      yes: true,
      dryRun: true,
  ...
  test('skill migration markdown exists at the expected path', async () => {
    ...
    const path = join(process.cwd(), 'skills/migrations/v0.11.2.md');
